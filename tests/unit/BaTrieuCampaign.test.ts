...
describe('GAME-C05 Bà Triệu production chapter', () => {
  it('registers exactly HBT then Bà Triệu with globally unique production IDs', () => {
    ...
    expect(new Set(productionCampaignCatalog.chapters.flatMap(({ stages }) => stages.map(({ id }) => id))).size).toBe(9)
  ...
  it('locks Bà Triệu until the final HBT production stage is completed', () => {
    ...
    const unlocked = { completedStages: { [HAI_BA_TRUNG_STAGE03_ID]: { firstCompletedAtMs: 2 } } }
  ...
  it('preserves ordered progression, completed replay and zero-playable safety', () => {
    let progress: CampaignProgressState = { completedStages: { [HAI_BA_TRUNG_STAGE03_ID]: { firstCompletedAtMs: 1 } } }
