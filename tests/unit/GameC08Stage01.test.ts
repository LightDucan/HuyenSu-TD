...
import { HAI_BA_TRUNG_STAGE_ID } from '../../src/data/campaign/haiBaTrungCampaign'
...
const stage = baTrieuChapter.stages[0]
const expectedComposition = [
  { 'wu-sword-infantry': 4 }, { 'wu-sword-infantry': 5 },
  { 'wu-sword-infantry': 3, 'wu-crossbow-soldier': 2 }, { 'wu-sword-infantry': 6, 'wu-crossbow-soldier': 3 },
  { 'wu-armored-guard': 2, 'wu-sword-infantry': 5 }, { 'wu-armored-guard': 2, 'wu-crossbow-soldier': 4, 'wu-sword-infantry': 4 },
  { 'wu-crossbow-soldier': 7 }, { 'wu-armored-guard': 3, 'wu-sword-infantry': 6 },
  { 'wu-armored-guard': 3, 'wu-crossbow-soldier': 5 }, { 'wu-sword-infantry': 8, 'wu-crossbow-soldier': 4 },
  { 'wu-armored-guard': 4, 'wu-sword-infantry': 6 }, { 'wu-armored-guard': 4, 'wu-crossbow-soldier': 6, 'wu-sword-infantry': 5 },
  { 'wu-sword-infantry': 10, 'wu-crossbow-soldier': 5 }, { 'wu-armored-guard': 5, 'wu-sword-infantry': 8 },
  { 'wu-armored-guard': 5, 'wu-crossbow-soldier': 8 }, { 'wu-armored-guard': 6, 'wu-sword-infantry': 8, 'wu-crossbow-soldier': 5 },
  { 'wu-armored-guard': 7, 'wu-crossbow-soldier': 8, 'wu-sword-infantry': 8 },
  { 'wu-field-commander': 1, 'wu-armored-guard': 6, 'wu-crossbow-soldier': 8, 'wu-sword-infantry': 10 },
]
...
describe('GAME-C08 Bà Triệu Stage 01 production contract', () => {
  it('keeps stable identity and exactly 18 exact, unique wave IDs', () => {
    expect(stage.id).toBe('bt-01-tu-nghia-nui-nua')
    expect(stage.waves).toHaveLength(18)
    expect(stage.waves.map(({ id }) => id)).toEqual(Array.from({ length: 18 }, (_, index) => `bt-01-wave-${String(index + 1).padStart(2, '0')}`))
    expect(new Set(stage.waves.map(({ id }) => id)).size).toBe(18)
  })
  ...
  it('matches the locked composition and reserves the generic field commander for Wave 18', () => {
    ...
    expect(stage.waves.flatMap((wave, index) => wave.groups.filter(({ enemyId }) => enemyId === 'wu-field-commander').map(() => index + 1))).toEqual([18])
  })
  ...
  it('keeps production spawn timing bounded without long empty tails', () => {
    stage.waves.flatMap(({ groups }) => groups).forEach(({ enemyId, startDelayMs, spawnIntervalMs }) => {
      ...
      const range = enemyId === 'wu-sword-infantry' ? [750, 950] : enemyId === 'wu-crossbow-soldier' ? [850, 1100] : enemyId === 'wu-armored-guard' ? [1100, 1500] : [1400, 1800]
      expect(spawnIntervalMs).toBeGreaterThanOrEqual(range[0]); expect(spawnIntervalMs).toBeLessThanOrEqual(range[1])
    })
    expect(stage.waves[0].groups[0].startDelayMs).toBe(0)
    expect(stage.waves[4].groups[0].startDelayMs).toBe(0)
    expect(stage.waves[17].groups[0].startDelayMs).toBe(0)
  })

  it('uses a dedicated bounded 1024x768 map with readable path and ten placements', () => {
    expect(stage.map).toMatchObject({ id: 'map-bt-nui-nua', width: 1024, height: 768, grid: { columns: 12, rows: 10 }, theme: 'nui-nua' })
    expect(stage.map.fixedPath.length - 1).toBeGreaterThanOrEqual(6)
  ...
  it('provides reconstruction-safe narrative with exact lightweight beats', () => {
    expect(stage.historicalConfidence).toBe(BA_TRIEU_STAGE01_HISTORICAL_CONFIDENCE)
    expect(stage.historicalConfidence).toMatch(/LOCAL TRADITION.*RECONSTRUCTION/)
    expect(stage.narrative?.preBattle).toMatch(/Production Reconstruction|truyền thống địa phương/i)
    expect(stage.narrative?.waveBeats.map(({ wave }) => wave)).toEqual([1, 6, 12, 18])
    expect(stage.narrative?.victory).toMatch(/đẩy lùi/i)
    expect(stage.narrative?.victory).not.toMatch(/tiêu diệt.*Đông Ngô|chiến tranh.*thắng/i)
  ...
  it('preserves the HBT prerequisite, Bà Triệu roster and Stage 01 boundary', () => {
    expect(baTrieuChapter.prerequisiteStageId).toBe(HAI_BA_TRUNG_STAGE_ID)
  ...
  it('keeps ordinary Stage 01 rewards and leaves Stages 02-06 as three-wave prototypes', () => {
