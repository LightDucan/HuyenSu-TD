...
import { productionHeroVisuals, resolveProductionHeroVisual, scaleVisualDuration } from '../../data/assets/prototypeVisualAssets'
...
import { resolveEnemyVisual } from '../../data/assets/enemyVisualAssets'
...
export class BattleScene extends Phaser.Scene {
  ...
  preload(): void {
    Object.values(productionHeroVisuals).forEach((visual) => {
    ...
    this.stage.enemyDefinitionIds.forEach((id) => { const visual = resolveEnemyVisual(id); if (visual?.walkUrl) this.load.spritesheet(visual.walkTextureKey, visual.walkUrl, { frameWidth: visual.frameWidth, frameHeight: visual.frameHeight }) })
  }
  ...
  create(): void {
    ...
    this.stage.enemyDefinitionIds.forEach((id) => { const visual = resolveEnemyVisual(id); if (!visual) return
  ...
  private spawnEnemy(definitionId: string): void {
    ...
    const visualDef = resolveEnemyVisual(definition.id)
  ...
  private onAttack(hero: PlacedHeroRuntime, targetId: string, damage: number, critical: boolean, killed: boolean): void {
    ...
    const visualAsset = resolveProductionHeroVisual(hero.definition.id)
  ...
  private createHeroRuntime(definition: HeroDefinition, position: Vector2, slotId: string): PlacedHeroRuntime {
    ...
    const visualAsset = resolveProductionHeroVisual(definition.id)
