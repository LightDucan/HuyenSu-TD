...
describe('GAME-C13 Bà Triệu Stage 06 final chapter contract', () => {
  ...
  it('preserves locked rewards and chapter completion semantics', () => {
    ...
    const before = { completedStages: { [HAI_BA_TRUNG_STAGE_ID]: { firstCompletedAtMs: 1 }, ...Object.fromEntries(BA_TRIEU_STAGE_IDS.slice(0, 5).map((id, index) => [id, { firstCompletedAtMs: index + 2 }])) } }
