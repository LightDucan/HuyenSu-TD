...
export function calculateHeroStats(base: HeroBaseStats, progression: HeroProgression, weapon: StatModifier = {}, gem: StatModifier = {}, star: HeroStar = 1, starGrowth: Partial<Record<HeroStar, Partial<HeroBaseStats>>> = {}): HeroBaseStats {
  ...
  return {
    ...
    atk: Math.round((base.atk * multiplier) + (weapon.atk ?? 0) + (gem.atk ?? 0)) + (growthStats.atk ?? 0),
    range: base.range + (weapon.range ?? 0) + (gem.range ?? 0) + (growthStats.range ?? 0),
    attackSpeed: base.attackSpeed + (weapon.attackSpeed ?? 0) + (gem.attackSpeed ?? 0) + (growthStats.attackSpeed ?? 0),
