...
describe('P11-C01 Reward Transaction Core', () => {
  ...
  it('commits Gold, KNB and consumable grants atomically in one revision', () => {
    ...
    expect(result).toMatchObject({ status: 'applied', save: { schemaVersion: 6, revision: 2 } })
  ...
  it('migrates valid V1 deterministically in the same key and keeps V1 raw data on migration failure', () => {
    ...
    expect(migrated).toMatchObject({ schemaVersion: 6, revision: 4, updatedAtMs: 1_000, data: { inventory: { unresolvedLegacyEquipmentInstanceIds: ['legacy-placeholder'] }, rewardReceipts: {}, activePlayTime: { observedVisibleMs: 0, observedHiddenMs: 0, remainderEligibleMs: 0 }, heroCollection: expect.any(Object) } })
    expect(repository.load()).toMatchObject({ status: 'loaded', save: { schemaVersion: 6, revision: 4 } })
  ...
  it('migrates V2 through V3 to V4 deterministically and preserves raw V2 when validation fails', () => {
    ...
    expect(migrated).toMatchObject({ schemaVersion: 6, revision: 7, updatedAtMs: 2_000, data: { inventory: { equipmentInstances: {}, equippedByHero: {} }, activePlayTime: { observedVisibleMs: 0, observedHiddenMs: 0, remainderEligibleMs: 0 }, heroCollection: expect.any(Object) } })
