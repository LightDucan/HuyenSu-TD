...
export function resolveSkill(skill: SkillDefinition, origin: Vector2, stats: HeroCombatStats, enemies: readonly CombatEnemy[]): SkillResult {
  ...
  const multiHit = skill.effects.find((effect): effect is Extract<SkillEffect, { type: 'multiHit' }> => effect.type === 'multiHit')
  const slow = skill.effects.find((effect): effect is Extract<SkillEffect, { type: 'slow' }> => effect.type === 'slow')
  const stun = skill.effects.find((effect): effect is Extract<SkillEffect, { type: 'stun' }> => effect.type === 'stun')
  const root = skill.effects.find((effect): effect is Extract<SkillEffect, { type: 'root' }> => effect.type === 'root')
  if (!damage && !slow && !stun && !root) return { affectedEnemyIds: [], killedEnemyIds: [] }
  ...
  targets.forEach((enemy) => {
    if (damage) enemy.hp = Math.max(0, enemy.hp - stats.atk * damage.atkMultiplier * (multiHit?.hits ?? 1))
    if (slow) enemy.slow = { ratio: slow.ratio, remainingMs: slow.durationMs }
    if (stun || root) enemy.immobilizedRemainingMs = Math.max(enemy.immobilizedRemainingMs ?? 0, stun?.durationMs ?? 0, root?.durationMs ?? 0)
