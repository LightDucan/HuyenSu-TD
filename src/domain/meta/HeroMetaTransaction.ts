...
export function applyHeroMetaOperation(save: MetaSaveV5, operation: HeroMetaOperation, config?: RecruitmentConfig): HeroMetaCommit {
  ...
  return { save: { ...save, revision: save.revision + 1, data: { ...save.data, heroCollection: next.heroCollection, inventory: { ...save.data.inventory, consumables: next.consumables } } }, results }
}

export function applyHeroMetaTransaction(save: MetaSaveV5, idempotencyKey: string, operation: HeroMetaOperation, config?: RecruitmentConfig): HeroMetaCommit {
  ...
  const applied = applyHeroMetaOperation(save, operation, config)
