...
describe('FAST-05A Hero Meta runtime', () => {
  it('upgrades one level in V5, publishes the snapshot, refreshes the placed Hero, and is idempotent', () => {
    ...
    const first = runtime.upgradeLevel('quan-vu', { expectedRevision: 1, idempotencyKey: 'hero/upgrade/1', committedAtMs: 100 })
    expect(first.save).toMatchObject({ revision: 2, updatedAtMs: 100, data: { heroCollection: { 'quan-vu': { progression: { level: 2 } } }, rewardReceipts: { 'hero/upgrade/1': { committedAtMs: 100 } } } })
    expect(snapshots).toHaveBeenCalledWith(first.save)
    expect(refreshes).toHaveBeenCalledWith('quan-vu')
    const retry = runtime.upgradeLevel('quan-vu', { expectedRevision: 2, idempotencyKey: 'hero/upgrade/1', committedAtMs: 100 })
    ...
    expect(() => runtime.upgradeLevel('quan-vu', { expectedRevision: 2, idempotencyKey: 'hero/upgrade/clock', committedAtMs: 99 })).toThrow('monotonic')
  })
  ...
  it('rejects free evolution and consumes Anh Hồn atomically while preserving stars', () => {
    ...
    const hero = initial.save.data.heroCollection['quan-vu']
    repository.save({ ...initial.save.data, heroCollection: { ...initial.save.data.heroCollection, 'quan-vu': { ...hero, stars: 4, progression: { stage: 'normal', level: 100 } } } }, 1, 10)
    const bridge = new BattleBridge()
    const refreshes = vi.fn(); bridge.onPlacedHeroStatsRefresh(refreshes)
    const runtime = new HeroMetaRuntimeController(repository, bridge)
    expect(() => runtime.evolve('quan-vu', { expectedRevision: 2, idempotencyKey: 'evolve/no-material', committedAtMs: 20 })).toThrow('Anh Hồn')
    ...
    const evolved = runtime.evolve('quan-vu', { expectedRevision: 3, idempotencyKey: 'evolve/paid', committedAtMs: 30 })
    expect(evolved.save.data.heroCollection['quan-vu']).toMatchObject({ stars: 4, progression: { stage: 'rebirth', level: 1 } })
  ...
  it('recruits through V5, makes a new Hero selectable, and blocks unowned deployment selection', () => {
    ...
    const starred = runtime.ascendStar('quan-vu', { expectedRevision: 3, idempotencyKey: 'star/2', committedAtMs: 3 })
    expect(starred.save.data.heroCollection['quan-vu'].stars).toBe(2)
    expect(refreshes).toHaveBeenCalledWith('quan-vu')
    expect(selectPlayableOwnedHeroIds(recruited.save.data.heroCollection, ['quan-vu', 'trieu-van'])).toEqual(['quan-vu'])
    expect(isHeroOwned(recruited.save.data.heroCollection, 'quan-vu')).toBe(true)
    expect(isHeroOwned(recruited.save.data.heroCollection, 'trieu-van')).toBe(false)
  })
})
