...
> - **Tuyệt đối KHÔNG thiết kế Enemy tấn công Hero**: Theo kiến trúc cốt lõi của Huyền Sử TD, **Enemy hiện chỉ di chuyển theo đường cố định (Fixed Path)** tiến về căn cứ/thành trì phe ta. Vũ khí của Enemy mang tính chất nhận diện thị giác (**Visual Identity**) và Lore bối cảnh, Enemy **KHÔNG CÓ cơ chế tấn công hay gây sát thương lên Hero**.
> - **Chưa khóa chỉ số Stats**: Không tự ý gán máu (HP), tốc chạy (Speed px/s) hay giáp số cụ thể; chỉ mô tả archetype định tính (`Chậm`, `Trung bình`, `Nhanh`, `Rất nhanh`, `Exact Speed = [CONFIG / OPEN]`).
...
> - **Quy chuẩn Asset**: 128 × 128 px, Front View only, 32-bit RGBA Transparent, Baseline tiếp đất **Y = 112 px**.
...
* **Hành vi trên đường đi (Path Behavior)**: Di chuyển chậm rãi (`Speed = Chậm [CONFIG / OPEN]`), bước đi đĩnh đạc và kiên cố, đóng vai trò lá chắn thịt đi đầu che chắn cho các đơn vị cơ động phía sau.
* **Tạo hình & Trang bị**:
  * Giáp phiến sắt sơn then đen bóng đan dây da bò chắc chắn, mũ sắt chỏm tròn có vành che gáy và má.
  * Tay trái mang Khiên chữ nhật lớn bằng gỗ bọc da nẹp sắt (Mộc thuẫn), tay phải cầm Kích sắt hoặc Hoàn Thủ đao ngắn (Visual identity).
...
* **Hành vi trên đường đi (Path Behavior)**: Di chuyển với tốc độ trung bình (`Speed = Trung bình [CONFIG / OPEN]`), đi theo cụm hỗ trợ phía sau các khối thiết giáp.
* **Tạo hình & Trang bị**:
  * Áo giáp nhẹ nẹp da đính đinh tán đồng, nón vải hoặc mũ sắt nhẹ có dây buộc cằm.
  * Hai tay mang Nỏ quân dụng gỗ cứng lẫy đồng, sau lưng đeo ống tên sắt đầu ba cạnh xuyên giáp (Visual identity).
* **Prompt Pixel Art (128×128 px, Baseline Y=112)**:
...
* **Hành vi trên đường đi (Path Behavior)**: Di chuyển nhanh (`Speed = Nhanh [CONFIG / OPEN]`), xuất hiện theo từng đàn đông đảo nhằm áp đảo hệ thống phòng thủ bằng số lượng.
* **Tạo hình & Trang bị**:
  * Áo chẽn ngắn màu xám đất, quần túm ống gọn gàng, đầu chít khăn vải thô.
  * Cầm câu liêm ngắn hoặc giáo tre vạt nhọn (Visual identity), cơ thể nhẹ nhàng linh hoạt khi lội qua bãi bồi đầm lầy.
* **Prompt Pixel Art (128×128 px, Baseline Y=112)**:
...
* **Hành vi trên đường đi (Path Behavior)**: Di chuyển với tốc độ rất nhanh (`Speed = Rất nhanh [CONFIG / OPEN]`), có khả năng bứt tốc lướt qua các đoạn đường cong nguy hiểm để tiếp cận cổng căn cứ nếu không bị dồn sát thương kịp thời.
* **Tạo hình & Trang bị**:
  * Cưỡi ngựa chiến Giang Đông dũng mãnh, ngực ngựa đeo yếm da bảo vệ.
  * Kỵ sĩ mặc giáp phiến sắt, đội mũ trụ đồng có gắn lông chim đỏ, tay cầm thương kích cán dài, cờ hiệu ngũ sắc tung bay sau lưng ngựa (Visual identity).
* **Prompt Pixel Art (128×128 px, Baseline Y=112)**:
...
* **Hành vi trên đường đi (Path Behavior)**:
  * Di chuyển chậm rãi, ung dung (`Speed = Chậm [CONFIG / OPEN]`) giữa toán quân hộ vệ.
  * Lượng máu (HP) dồi dào, là thử thách lớn nhất cho toàn bộ hệ thống Hero phòng thủ.
* **Tạo hình Visual**:
  * Mặc cẩm bào bằng lụa quý Giang Đông màu tím thẫm bên trong giáp phiến sắt viền đồng mạ vàng uy nghi.
  * Đội mũ quan võ thời Tam Quốc cánh chuồn chạm vàng, tay cầm thanh kiếm Hoàn Thủ chuôi vàng nạm ngọc (Visual identity).
...
* **Hành vi trên đường đi (Path Behavior)**:
  * Di chuyển trên chiến xa nặng nề với tốc độ rất chậm (`Speed = Rất chậm [CONFIG / OPEN]`).
  * Khả năng chống chịu đòn đánh vật lý cao.
* **Tạo hình Visual**:
  * Tướng béo tốt mang giáp sắt dày cộp, trước ngực đeo gương hộ tâm phiến tròn lớn.
  * Đứng trên Chiến xa gỗ bọc sắt nẹp các thanh cọc nhọn đâm tua tủa xung quanh (Visual identity).
* **Prompt Pixel Art (128×128 px, Baseline Y=112)**:
