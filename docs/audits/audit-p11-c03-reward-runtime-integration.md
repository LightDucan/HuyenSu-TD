...
- Production Active Play rewards use the locked 60,000 ms real-time interval; this is independent from Command Energy's two-minute regeneration rule.
...
- A new tracker starts from the persisted checkpoint after reload and cannot reclaim previously observed elapsed time.
- Every Phaser scene `init()` creates a fresh Battle run ID, so restart/replay uses a new Stage Clear idempotency key.
...
- Tracker reload continues from the persisted checkpoint: PASS.
- Visible → hidden → visible attribution: PASS.
- Save/reload cannot reclaim old elapsed time: PASS.
- Optimistic revision conflict protection: PASS.
- Scene restart/replay run identity: PASS — each `init()` receives a fresh ID.
